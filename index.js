...
app.post("/api/getKiteOrders", async (req, res) => {
	try {
		...
		if (!fetchedOrders.status) {
			res
				.status(400)
				.send(JSON.stringify({ success: false, error: fetchedOrders.error }));
		} else {
			res
				.status(200)
				.send(
					JSON.stringify({ success: true, orders: fetchedOrders.data["data"] })
				);
		}
	} catch (err) {
		console.log(err);
...
app.post("/api/getKitePositions", async (req, res) => {
	try {
		...
		if (!fetchedOrders.status) {
			...
			res
				.status(200)
				.send(JSON.stringify({ success: true, data: fetchedOrders.data }));
		}
	} catch (err) {
		console.log(err);
		res.status(500).send(JSON.stringify({ success: false, error: err }));
